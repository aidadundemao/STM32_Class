...
连。注意：需先安装“开发板USB转串口CH340驱动软件CH341SER.EXE”软件。
	...
	四、本项目的功能
	通过fputc()函数实现串口的printf打印功能。
    实现USART的printf功能只要做好两个工作就可以了：一个是串口的初始化，另一个就是两个函数fputc()、
fgetc()的重定向。
    注意， 打开“魔法棒”之后，在“Target”界面上在“MicroLib”之前的方框内打勾，即使能。
	
	五、串口初始化的步骤
	1.初始化I/O口和时钟(用于串行通信引脚)。函数GPIO_Init()，函数RCC_APB2PeriphClockCmd()。
	2.初始化串口及时钟。函数USART_Init()，函数RCC_APB2PeriphClockCmd()。
	3.使能串口,即开启串口功能。函数USART_Cmd()。