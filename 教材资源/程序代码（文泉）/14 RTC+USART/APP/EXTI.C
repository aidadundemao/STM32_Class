...
void EXTIInti()
{
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOE, GPIO_PinSource3); //PE2:second+
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 3;//配置抢占式优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;//配置响应式优先级
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE; //中断使能
	NVIC_Init(&NVIC_InitStructure); 	
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 3;//配置抢占式优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;//配置响应式优先级
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE; //中断使能
	NVIC_Init(&NVIC_InitStructure); 	
