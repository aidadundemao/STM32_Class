...
void USART1_IRQHandler(void)	//串口1中断函数
{
	...
	//收到完整的时分秒数值后 将其写入到实时钟寄存器当中
	if(k%3==0) 
	{
		RTC_SetCounter(hour*3600+minute*60+second+1);//设置RTC计数器的值 +1修正通信延时
...
void RTC_IRQHandler()
{
	...
	if(RTC_GetITStatus((RTC_IT_SEC))!=RESET)
	{
		...
		RTCNum=RTC_GetCounter();//获取RTC计数器的值
		hour=(RTCNum/3600)%24;		  //以秒为单位计算时间
		minute=(RTCNum%3600)/60;
		second=RTCNum%60;
	}
}
